...
using SolutionTemplate2.BackEnd.Infrastructure.Database.Interceptors;
using SolutionTemplate2.BackEnd.Infrastructure.Database.Seeders;

namespace SolutionTemplate2.BackEnd.Infrastructure.Database;
    ...
    public static IServiceCollection AddDatabaseService(this IServiceCollection services, string connectionString, IHealthChecksBuilder healthChecksBuilder)
    {
        _ = services.AddDbContext<IDatabaseService, DatabaseService>(options =>
        {
            _ = options.UseSqlServer(connectionString, builder =>
            {
                ...
                _ = builder.MigrationsHistoryTable("__EFMigrationsHistory", nameof(SolutionTemplate2));
