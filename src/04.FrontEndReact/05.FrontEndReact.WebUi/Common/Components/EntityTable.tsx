  ...
  return (
    <div className="w-full overflow-hidden">
      ...
      <div className="max-h-[calc(100svh-8rem)] overflow-auto">
        <table className="w-full text-left text-sm [&_tbody_tr:hover]:bg-zinc-50">
          <thead className="sticky top-0 z-10 bg-muted text-muted-foreground">
            <tr>
              {headers.map((header, index) => {
                ...
                return (
                  <th key={`${label}-${index}`} className={`px-3 py-2 font-medium ${className ?? (isCenteredColumn ? "text-center" : "")}`} style={width ? { width } : undefined}>
                    {label}
                  </th>
                );
              })}
